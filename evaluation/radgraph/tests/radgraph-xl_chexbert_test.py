"""Pytest to ensure RadGraph‑XL predictions are stable on the CheXbert test set.

Run with:
    pytest -xvs tests/test_radgraph_xl.py

If the test fails after intentionally upgrading RadGraph‑XL or the
pre‑/post‑processing logic, regenerate
``chexbert_test_set_radgraph_xl.json`` and commit the updated file **only
once you have manually inspected the diff and confirmed the changes are
expected**.
"""
...
@pytest.mark.parametrize("model_type", ["radgraph-xl"])
def test_radgraph_xl_outputs_match_expected(model_type):
    """Verify current model output matches the stored golden file."""
    base_dir = os.path.dirname(__file__)
    expected_path = os.path.join(base_dir, "chexbert_test_set_radgraph_xl.json")
