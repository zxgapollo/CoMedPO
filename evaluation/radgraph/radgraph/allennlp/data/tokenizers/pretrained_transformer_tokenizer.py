...
def _call_encode_plus(tokenizer, *args, **kwargs):
    """Compatibility wrapper for tokenizer.encode_plus which may be removed in newer transformers.
    Falls back to tokenizer.__call__ or tokenizer._encode_plus if necessary.
    """
    if hasattr(tokenizer, "encode_plus"):
        return tokenizer.encode_plus(*args, **kwargs)
    if hasattr(tokenizer, "_encode_plus"):
        return tokenizer._encode_plus(*args, **kwargs)
    if hasattr(tokenizer, "__call__"):
        # transformers' __call__ returns a BatchEncoding similar to encode_plus
        return tokenizer(*args, **kwargs)
    raise AttributeError("Tokenizer has no encode_plus/_encode_plus/__call__ method")
...
class PretrainedTransformerTokenizer(Tokenizer):
    ...
    def _reverse_engineer_special_tokens(
        ...
        dummy_output = _call_encode_plus(tokenizer_with_special_tokens,
            token_a,
            token_b,
    ...
    def tokenize(self, text: str) -> List[Token]:
        ...
        """
        encoded_tokens = _call_encode_plus(self.tokenizer,
            text=text,
    ...
    def _intra_word_tokenize(
        ...
        for token_string in string_tokens:
            wordpieces = _call_encode_plus(self.tokenizer,
                token_string,
                add_special_tokens=False,
                return_tensors=None,
