...
class PretrainedTransformerIndexer(TokenIndexer):
    ...
    def _postprocess_output(self, output: IndexedTokenList) -> IndexedTokenList:
        ...
        if self._max_length is not None:
            ...
            def _build_inputs_with_special_tokens(tok, ids):
                # Try tokenizer API compatibly across transformers versions.
                ...
                # Fallback: return ids unchanged (no special tokens added)
                return ids

            folded_indices = [
                _build_inputs_with_special_tokens(self._tokenizer, segment)
