...
class RadGraph(nn.Module):
    def __init__(
        ...
        # Load state dict with strict=False to allow loading checkpoints that may
        # not contain certain buffers such as position_ids depending on tokenizer/model variants.
        try:
            model.load_state_dict(model_state, strict=False)
        except Exception:
            # Fallback to strict=True if desired (preserve original behavior on failure)
