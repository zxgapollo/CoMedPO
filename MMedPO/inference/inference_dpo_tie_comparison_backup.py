...
def calculate_dpo_weight(delta_pos, delta_neg, m_v=None, m_n=None, delta_obj=None, 
                        ...
                        tau_flip=0.3, rho=0.1, batch_stats=None):
    """
    计算基于TIE特征的DPO权重
    ...
    Args:
        ...
        tau_flip: 翻转阈值
        rho: 翻转时的权重缩放因子
        batch_stats: 批统计信息 (mu, sigma, tau_n)
    ...
    return {
        ...
        'preference_prob': p,
        'dpo_weight': round(w, 4),
        'flipped': delta_neg - delta_pos > tau_flip
    }

def calculate_batch_statistics(all_results, tau_n_percentile=80):
    """
    计算批统计信息用于DPO权重标准化
    ...
    """
    if not all_results:
        return {'mu': 0.0, 'sigma': 1.0, 'tau_n': 0.0}
...
def build_tie_anker_dpo_pairs(results, args):
    ...
    for result in results:
        ...
        # 应用阈值过滤
        if apply_tie_anker_thresholds(tie_weight, tie_diff_norm, result, args):
            # 构建DPO对
            dpo_pair = {
                "id": result.get('id'),
                "case_id": result.get('case_id'),
                "question": result.get('question'),
                "image": result.get('case_id', '') + "/" + result.get('case_id', '') + ".jpg",  # 假设图片路径格式
                "chosen": result.get('positive_answer'),
                "rejected": result.get('negative_answer'),
                "tie_weight": float(tie_weight),
                "tie_score": float(tie_diff_norm),
                "validation_info": {
...
def eval_model(args):
    ...
    for i, line in enumerate(progress_bar):
        ...
        if "image" in line:
            ...
            full_image_path = os.path.join("/workspace/MMedPO/datasets/Slake1.0/imgs", image_path)
