...
def eval_model(args):
    ...
    for line in tqdm(dataloader, position=0, file=sys.stdout):
        ...
        if "qid" in line:
            ...
            if not isinstance(question_obj, dict) or "value" not in question_obj:
                tqdm.write(f"Warning: Invalid question_obj for item {idx}. Skipping.")
                continue
        ...
        # Optionally save the composite image for verification
        if getattr(args, "output_image_folder", None) and rank == 0:
            try:
                ...
                output_path = os.path.join(output_dir, f"{base_name}_with_context_composite.jpg")
                composite_image.save(output_path)
            except Exception as e:
                tqdm.write(f"Warning: Could not save composite image for {case_id}. Reason: {e}")

        # Prompt with full context (original question)
        qs = (
            "The following image displays a full medical scan on the left and a version with a masked background on the right. "
...
def main():
    ...
    parser.add_argument("--output-image-folder", type=str, default=None, help="Optional: Path to save composed images for with-context runs.")
