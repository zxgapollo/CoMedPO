...
import math
from transformers import set_seed, logging, AutoImageProcessor, CLIPImageProcessor
...
def force_resize_to_224(image):
    """Resize a PIL image to 224x224 using bicubic interpolation.
    If conversion fails, ensure RGB mode first.
    """
    try:
        return image.resize((224, 224), Image.BICUBIC)
    except Exception:
        return image.convert('RGB').resize((224, 224), Image.BICUBIC)
...
def run_inference_with_image(model, tokenizer, image_processor, composite_image, prompt_text, target_answer, args, device):
    ...
    # Ensure composite image is 224x224
    composite_image = force_resize_to_224(composite_image)
...
def eval_model(args):
    ...
    if image_processor is None:
        print(f"Rank {rank}: Image processor was not loaded; attempting to load from model_path...")
        try:
            image_processor = AutoImageProcessor.from_pretrained(model_path)
            print(f"Rank {rank}: Loaded image processor from model_path")
        except Exception as e:
            print(f"Rank {rank}: AutoImageProcessor load failed: {e}")
            try:
                image_processor = CLIPImageProcessor.from_pretrained('openai/clip-vit-large-patch14-336')
                print(f"Rank {rank}: Loaded CLIP image processor fallback")
            except Exception as e2:
                print(f"Rank {rank}: Failed to load CLIP image processor: {e2}")
                print(f"Rank {rank}: Continuing without image processor...")
                image_processor = None

    # Enforce 224x224 sizing on the processor if available
    if image_processor is not None:
        try:
            # Normalize to a valid key set to avoid invalid combinations like
            # {'shortest_edge', 'height', 'width'} which HF rejects.
            new_size = {'height': 224, 'width': 224}

            if hasattr(image_processor, 'size'):
                try:
                    # Prefer replacing the entire dict to guarantee valid keys
                    image_processor.size = new_size
                except Exception:
                    size_attr = getattr(image_processor, 'size')
                    if isinstance(size_attr, dict):
                        size_attr.clear()
                        size_attr.update(new_size)
                    else:
                        setattr(image_processor, 'size', new_size)

            if hasattr(image_processor, 'crop_size'):
                try:
                    image_processor.crop_size = new_size.copy()
                except Exception:
                    crop_attr = getattr(image_processor, 'crop_size')
                    if isinstance(crop_attr, dict):
                        crop_attr.clear()
                        crop_attr.update(new_size)
                    else:
                        setattr(image_processor, 'crop_size', new_size.copy())
        except Exception as e:
            print(f"Rank {rank}: Warning: could not enforce 224x224 on processor: {e}")
    ...
    try:
        for i, line in enumerate(progress_bar):
            ...
            if "image" in line:
                ...
                # 原图与掩膜根路径由 CLI 传入，避免硬编码
                imgs_root = getattr(args, 'imgs_root', None) or "/workspace/MMedPO/datasets/SLAKE/imgs"
                processed_imgs_root = getattr(args, 'processed_imgs_root', None) or "/workspace/MMedPO/datasets/SLAKE/processed_imgs"
                # 原图定位到 imgs_root
                full_image_path = os.path.join(imgs_root, image_path)
                # 掩膜图定位到 processed_imgs_root（保持相同的相对路径）
                masked_image_path = os.path.join(processed_imgs_root, image_path)
            else:
        ...
        interrupted = True
        progress_bar.write("KeyboardInterrupt received; saving partial results and exiting gracefully.")
    ...
    if world_size > 1 and not interrupted:
        dist.barrier()
        ...
        if rank == 0:
            ...
            for res in all_results:
                ...
                if q_id not in seen_ids:
                    ...
                    seen_ids.add(q_id)
    elif world_size > 1 and interrupted:
        # Graceful path: skip distributed gather, use local results
        print(f"Rank {rank} interrupted; using local results without distributed gather.")
...
def main():
    ...
    # 图像根路径参数（避免硬编码）
    parser.add_argument("--imgs-root", type=str, default="/workspace/MMedPO/datasets/SLAKE/imgs", help="Root directory for original images")
    parser.add_argument("--processed-imgs-root", type=str, default="/workspace/MMedPO/datasets/SLAKE/processed_imgs", help="Root directory for processed/masked images")
