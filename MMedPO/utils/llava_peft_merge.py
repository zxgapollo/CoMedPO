...
def merge_lora_with_base_model(base_model_path, lora_checkpoint_path, output_path, device_map="auto"):
    ...
    try:
        ...
        if "Mistral" in model_architecture:
            ...
            # The tokenizer loading is consistently failing with AutoTokenizer due to model type mismatch.
            # We will bypass the auto-detection and explicitly use the slow LlamaTokenizer,
            # which is more robust to the parsing error we are seeing.
            print("🔹 Bypassing AutoTokenizer. Loading with explicit LlamaTokenizer...")
...
from transformers import (
    ...
    AutoTokenizer,
    BitsAndBytesConfig,
    LlamaTokenizer,  # Add LlamaTokenizer import
    LlavaConfig,
