...
# SPPO-ADV 训练脚本（公式版）：loss_variant=sppo_adv_squared，
# 使用 lambda 加权正/负两项的平方损，并加入 η 概率塑形
...
OUTPUT_DIR="/workspace/MMedPO/MMedPO/checkpoints/sppo_adv_model"
...
echo "=== Starting SPPO-ADV Training ==="
...
echo "beta=$BETA"
echo "=================================="
echo "loss_variant=sppo_adv_squared, sppo_lambda=$SPPO_LAMBDA, sppo_eta=$SPPO_ETA"
...
python3 train_dpo_dual_gpu.py \
    ...
    --beta $BETA \
    --loss_type sigmoid \
    --loss_variant sppo_adv_squared \
    --sppo_lambda $SPPO_LAMBDA \
    --sppo_eta $SPPO_ETA


echo "SPPO-ADV (公式版) training completed. Model saved to: $OUTPUT_DIR"