...
# TIE-SPPO (Tie-Shaped Preference Policy Optimization) Training Script
...
OUTPUT_DIR="/workspace/MMedPO/MMedPO/checkpoints/tie_sppo_model"
...
echo "=== Starting TIE-SPPO Training ==="
...
echo "beta=$BETA"
echo "=================================="
echo "loss_variant=tie_sppo, sppo_alpha=$SPPO_ALPHA, sppo_eta=$SPPO_ETA"
...
python3 train_dpo_dual_gpu.py \
    ...
    --beta $BETA \
    --loss_type sigmoid \
    --loss_variant tie_sppo \
    --sppo_alpha $SPPO_ALPHA \
    --sppo_eta $SPPO_ETA


echo "TIE-SPPO training completed. Model saved to: $OUTPUT_DIR"