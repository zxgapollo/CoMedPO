...
| Supplement Eq. (23), causal ratio weighting | Related TIE weighting experiments exist | Kept separate from the main-paper objective |
| Training settings, main Section 5 | Several historical scripts and environment-specific configurations exist | LoRA, optimizer, and epoch defaults follow the reported training settings |
...
The objective follows main-paper Eqs. (11)-(14), not the causal-ratio weighting ablation in supplement S6.1. The training entry point exposes configurable hyperparameters and an explicit data contract.
...
Local validation uses a real tiny randomly initialized LLaVA-Mistral model and local CLIP tower, in addition to loss/data regression tests.
